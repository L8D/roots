...
gh_pages.check_install_status = function(cb){
  ...
  original_branch = shell.exec('git rev-parse --abbrev-ref HEAD').output.trim();
...
gh_pages.push_code = function(cb){
  ...
  var public_folder = roots.project.dirs['public'];
  ...
  shell.exec('git checkout ' + original_branch);
